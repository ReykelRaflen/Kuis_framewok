        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('berita.index')" :active="request()->routeIs('berita.index')" class="text-gray-700 hover:text-blue-600">
                        {{ __('Beranda') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            ...
                            @if(Auth::user()->isAdmin())
                                <span class="ml-2 text-xs bg-red-500 text-white px-2 py-1 rounded-full">Admin</span>
                            @endif
                            <div class="ms-1">
                    ...
                    <x-slot name="content">
                        @if(Auth::user()->isAdmin())
                            <x-dropdown-link :href="route('berita.create')" class="text-blue-600">
                                {{ __('Tulis Berita') }}
                            </x-dropdown-link>
                        @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4">
                ...
                @if(Auth::user()->isAdmin())
                    <span class="text-xs bg-red-500 text-white px-2 py-1 rounded-full">Admin</span>
                @endif
            </div>

            <div class="mt-3 space-y-1">
                @if(Auth::user()->isAdmin())
                    <x-responsive-nav-link :href="route('berita.create')">
                        {{ __('Tulis Berita') }}
                    </x-responsive-nav-link>
                @endif
